...
export const DineInPage: React.FC = () => {
      ...
      if (table && table.status === "occupied") {
        ...
        if (order && order.items) {
          ...
          const cartItems: CartItem[] = order.items.map((item: any) => ({
            ...
            printedQuantity: item.sentToKitchen ? item.quantity : (item.printedQuantity || 0)
          }));
  ...
  const generateBillContent = useCallback(() => {
    ...
    // Get all items from current order (which already contains table order items when loaded)
    ...
    return content;
  }, [getAllCombinedItems, billConfig, selectedTableData?.name, existingTableOrder, restaurantSettings]);
  ...
  const placeOrder = useCallback(async () => {
    ...
    try {
      ...
      // Update printed quantities in currentOrder
      ...
      if (selectedTable && selectedTableData) {
        ...
        if (order && order.items) {
          const cartItems: CartItem[] = order.items.map((item: any) => ({
            ...
            sentToKitchen: item.sentToKitchen || false,
            department: item.department,
            printedQuantity: item.quantity  // Mark as printed since we just sent to kitchen
          }));
          setCurrentOrder(cartItems);
        }
  ...
  }, [getPendingItems, kotConfig, printKOT, printKOTByDepartment, selectedTable, selectedTableData, addItemsToTable, markItemsAsSent, getTableOrder]);
              ...
              {currentOrder.length === 0 ? (
                ...
                <div className="space-y-4">
                  ...
                  {currentOrder.map((it, idx) => (
                    <div
                      key={`${it.id}-${idx}`}
                      className="flex items-start justify-between p-5 border-2 border-gray-200 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors"
                    >
            ...
            {getAllCombinedItems().length > 0 && (
              <div className="mb-4">
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto">
                  {getAllCombinedItems().map((item, idx) => (
